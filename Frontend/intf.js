...
async function prodPopulate(catid, name) {
    try{
        if(catid == 0) {
            ...
            banner.innerHTML = `<div id="banner">Welcome to IPC Shop! You are browsing: Category / Hello, ${uname}, not you? <a href= "lgportal.html">Login Here</a> <a href="checkout.html"><img class="cart" src="spc.jpg"></a></div>`
            ...
            <br>
            Important Note: This whole webpage is for the course IERG4210 offered by CUHK @2025 spring semester as project submission, do NOT expect to be able to make legit purchase, in case that's what you are thinking to achieve, for whatever reason it is.
            <iframe src="https://www.facebook.com/plugins/like.php?href=https%3A%2F%2Fwww.facebook.com%2Fshare%2F1NZ9NW1sXf%2F%3Fmibextid%3DwwXIfr&width=20&layout&action&size&share=true&height=35&appId" width="20" height="35" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share"></iframe>
        </section>`;
        spinit();
        renderlist();
        }
	    ...
	    //console.log(products);
            //console.log(rp);
	   // console.log(reply);
            banner.innerHTML = `<div id="banner">Welcome to IPC Shop! You are browsing: Category / Hello, ${uname}, not you? <a href= "lgportal.html">Login Here</a> / <a href= "/authv1/logout">Logout Here</a> <a href="checkout.html"><img class="cart" src="spc.jpg"></a></div>`
            data.innerHTML = `<section class="viewitem">
            <div class="shopping-list" id="shoppingList">
                <h3>Shopping List (Hover For Details)</h3>
                <ul id="cartItems"></ul>
            ...
            products.forEach((prod) => {
                ...
                <div class="goods">
                    ...
                    <p>$${prod.price}</p>
                    <button onclick="addcart(${prod.pid}, '${prod.name}', ${prod.price})">Add to Cart</button>
                </div>`;
        });
        prodDet += '</div>';
...
function addcart(pid, product, price) { // if matching, +1 quantity, else push item to last of spcart
    const exist = spcart.find(item => item.pid === pid);
    if (exist) {
        exist.qty++;
    } else {
        spcart.push({ pid, name: product, price: parseFloat(price), qty: 1 });
    }
    renderlist();
    spcartstring();
}
